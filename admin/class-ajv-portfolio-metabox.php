...
class AJV_Portfolio_Metabox {
	...
	public function save_meta_box_values( $post_id ) {
		...
		$is_valid_nonce = ( isset( $_POST['ajv_portfolio_details_nonce'] ) && wp_verify_nonce( sanitize_key( $_POST['ajv_portfolio_details_nonce'] ), basename( __FILE__ ) ) ) ? 'true' : 'false';
		...
		if ( isset( $_POST['_ajv_portfolio_description'] ) ) {
			update_post_meta( $post_id, '_ajv_portfolio_description', sanitize_textarea_field( wp_unslash( $_POST['_ajv_portfolio_description'] ) ) );
		}

		if ( isset( $_POST['_ajv_portfolio_client'] ) ) {
			update_post_meta( $post_id, '_ajv_portfolio_client', sanitize_text_field( wp_unslash( $_POST['_ajv_portfolio_client'] ) ) );
		}

		if ( isset( $_POST['_ajv_portfolio_company'] ) ) {
			update_post_meta( $post_id, '_ajv_portfolio_company', sanitize_text_field( wp_unslash( $_POST['_ajv_portfolio_company'] ) ) );
		}

		if ( isset( $_POST['_ajv_portfolio_url'] ) ) {
			update_post_meta( $post_id, '_ajv_portfolio_url', esc_url_raw( wp_unslash( $_POST['_ajv_portfolio_url'] ) ) );
		}

		if ( isset( $_POST['_ajv_portfolio_url_text'] ) ) {
			update_post_meta( $post_id, '_ajv_portfolio_url_text', sanitize_text_field( wp_unslash( $_POST['_ajv_portfolio_url_text'] ) ) );
		}
